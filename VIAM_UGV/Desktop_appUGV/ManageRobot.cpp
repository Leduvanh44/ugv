...
//            currRealHeading[0] = 0;
        ...
        mqttClient->setHostname(ui->IPAddress_UGV1->text());
        mqttClient->setPort(ui->Port_UGV1->text().toInt());
        ui->Page_robots->findChild<QLabel *>(status)->setStyleSheet("background-color: rgb(0, 255, 0); ");
        if (ui->Connect_UGV1->text() == "Connect") {
                ui->Port_UGV1->setEnabled(false);
                ui->IPAddress_UGV1->setEnabled(false);
                ui->Connect_UGV1->setText(tr("Disconnect"));
                mqttClient->connectToHost();
                ui->Status_UGV1->setStyleSheet(" QLabel{background-color: green;border-radius: 20px; border: 3px solid rgb(0, 130, 0);}");

        } else {
                ui->Port_UGV1->setEnabled(true);
                ui->IPAddress_UGV1->setEnabled(true);
                ui->Connect_UGV1->setText(tr("Connect"));
                mqttClient->disconnectFromHost();
                ui->Status_UGV1->setStyleSheet("QLabel{background-color: red;border-radius: 20px; border: 3px solid rgb(100, 0, 0);}");
            }
//        }
...
{
    if (ui->Start_UGV1->text() == "Start") {
//    start_cmd = 0x31;
//    listaddr_robot.clear();
//    Listlocation.append("1");
